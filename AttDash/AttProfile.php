<?php
    ...
    $query = "SELECT Emp_fname, Emp_lname, Flight_Att.Emp_Password, Att_Status, Att_Hours, Att_Rank FROM Flight_Att, Employee WHERE Flight_Att.Emp_ID = Employee.Emp_ID AND Flight_Att.Emp_ID = ".$_SESSION['Emp_ID'];
    $result = mysqli_query($link, $query);
    $attInfo = mysqli_fetch_row($result);

    $DBName = $attInfo[0];
    $DBLName = $attInfo[1];
    $DBPassword = $attInfo[2];
    $DBStatus = $attInfo[3];
    $DBHours = $attInfo[4];
    $DBRank = $attInfo[5];
?>
...
/*                font-size: 20pt;*/
    ...
    <body>
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="myNavbar">
                    <ul class="nav navbar-nav">
                        <li><a href="FlightAttendant.php">Dashboard</a></li>
                    </ul>
                    <ul class="nav navbar-nav navbar-right">
                        <li class="active"><a href="AttProfile.php"><span class="glyphicon glyphicon-user"></span> Edit Profile</a></li>
        ...
        <div class="row">
            <form method="POST" action="AttProfile.php" class="form-horizontal">
...
<!--                    <div class="col-md-9"><input type="text" name="Rank" class="form-control" id="Rank" <?php echo "value='". $DBRank ."'"; ?>></div>-->
                    <div class="col-md-9" id="mainselection">
                        <select name="Rank" id="Rank">
                            <option value="Junior" <?php if($DBRank == "Junior") {echo 'selected="selected"';} ?>>Junior</option>
                            <option value="Senior" <?php if($DBRank == "Senior") {echo 'selected="selected"';} ?>>Senior</option>
                        </select>
                    </div>
                </div>
        ...
        <?php
            if(isset($_POST['submit'])) {
                ...
                $attUpdate = "UPDATE Flight_Att SET Emp_Password = ?, Att_Status = ?, Att_Hours = ?, Att_Rank = ? WHERE Emp_ID = ?";
                if($stmt = mysqli_prepare($link, $employeeUpdate)) {
                    ...
                    if(mysqli_stmt_execute($stmt)) {
                        if($stmt = mysqli_prepare($link, $attUpdate)) {
                            mysqli_stmt_bind_param($stmt, 'siisi', $_POST['Password'], $_POST['Status'], $_POST['Hours'], $_POST['Rank'], $_SESSION['Emp_ID']) or die("<div class='well'>Attendant Bind failed</div>");
                            if(mysqli_stmt_execute($stmt)) {
                                date_default_timezone_set('America/Chicago');
                                $sql = "INSERT INTO Log(Log_IP_Address, Log_Date, Log_Time, Log_Action, Log_User, Emp_ID) VALUES('".$_SERVER['REMOTE_ADDR']."', '".date('Y-m-d')."', '".date('H:i:s')."', 'PROFILE UPDATE', 'ATTENDANT', ".$_SESSION['Emp_ID'].")";
                                ...
                                echo "<div class='well'>User ". $_SESSION['Emp_ID'] ." has been updated</div>";
                                header("Location: AttProfile.php");
                                mysqli_close($link);
                                exit;
                            }
                            ...
                            echo "<div class='well'>Attendant prepare failed</div>";
                        }
                    }
                }
                else {
